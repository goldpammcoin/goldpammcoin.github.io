  ...
  getBalances: function() {
    ...
    App.contracts.TutorialToken.deployed().then(function(instance) {
      tutorialTokenInstance = instance;
      App.addresses.constract=tutorialTokenInstance.address
      return tutorialTokenInstance.totalBalance();
    }).then(function(result) {
      balance= new BigNumber(result).div(App.wei).toFixed(6);
      //balance = result.c[0];
    ...
    App.contracts.TutorialToken.deployed().then(function(instance) {
      ...
      balance= new BigNumber(result[0] + result[1]).div(App.wei).toFixed(6);
      $('#totalValue').text(balance);
    }).catch(function(err) {
      console.log(err.message);
    });
